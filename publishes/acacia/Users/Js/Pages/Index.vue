<template>
    ...
    <Backend>
        <template #header>
            <h4 class="font-black text-2xl px-1 md:px-4">Users</h4>
        </template>
                        ...
                        <template #body="{ data }">
                            <span>{{
                                data.email_verified_at
                                    ? dayjs(data.email_verified_at).format(
                                          "MMM DD, YYYY hh:mm a"
                                      )
                                    : "-"
                            }}</span>
        ...
        >
            <template #header>
                <h4 class="font-black text-xl">New User</h4>
            </template>
        ...
        >
            <template #header>
                <h4 class="font-black text-xl">User Details</h4>
            </template>
        ...
        >
            <template #header>
                <h4 class="font-black text-xl">Edit Single User</h4>
            </template>
            <EditForm
                :model="currentModel"
                @updated="onUpdated"
...
const fetchModel = async (row) => {
    axios
        .get(route("api.v1.users.show", row))
        .then((res) => {
...
const deleteModel = async function (row) {
    try {
        const res = await axios.delete(
            route("api.v1.users.destroy", row as any)
        );
        toast.add({
            severity: "success",
