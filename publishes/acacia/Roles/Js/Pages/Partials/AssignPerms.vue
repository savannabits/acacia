<template>
    <div>
        <div
            class="font-black text-xl mb-2 flex flex-wrap justify-between items-center"
        >
        ...
        <div class="p-4 mt-2 md:grid grid-cols-2 border rounded">
            <div
                v-for="(perms, groupIndex) of permissions"
                :key="groupIndex"
            ...
            >
                <div class="flex space-x-2 flex-wrap">
                    ...
                    <div class="space-x-2">
                        <Button
                            @click.prevent="togglePermGroup(perms, true)"
                            class="p-button-success p-button-sm text-sm"
                            label="Assign"
                            icon="pi pi-check"
                        />
                        <Button
                            @click.prevent="togglePermGroup(perms, false)"
                            class="p-button-danger p-button-sm text-sm"
                            label="Revoke"
                            icon="pi pi-ban"
                        />
                    </div>
                ...
                >
                    ...
                    <label
                        ...
                        class="inline-block font-black text-xl ml-3"
                        >{{ perm.title }}</label
                    >
                </div>
            </div>
...
const togglePermGroup = async (permGroup, checked) => {
    ...
    emit(
        "group-toggle-success",
        `All permissions in the group have been ${
            checked ? "Assigned." : "Revoked."
        }`
    );
};
</script>
